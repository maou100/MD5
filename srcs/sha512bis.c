...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/03 15:41:22 by feedme            #+#    #+#             */
/*   Updated: 2019/05/03 15:46:11 by feedme           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "md5.h"

unsigned long	r_rot64(unsigned long f, unsigned int s)
{
	return ((f >> s) | (f << (64 - s)));
}
...
unsigned char	*prepare_sha512(unsigned char *inp, int len)
{
	...
	new[len] = 128;
	tmp2 = (len * 8);
	ft_memrev(&new[new_len - 8], &tmp2, 8);
	return (new);
}
...
unsigned long	*init_sha512(t_sha512 *vars, unsigned char *inp,
unsigned int len)
{
	vars->h0 = 0x6a09e667f3bcc908;
	vars->h1 = 0xbb67ae8584caa73b;
	vars->h2 = 0x3c6ef372fe94f82b;
	vars->h3 = 0xa54ff53a5f1d36f1;
	vars->h4 = 0x510e527fade682d1;
	vars->h5 = 0x9b05688c2b3e6c1f;
	vars->h6 = 0x1f83d9abfb41bd6b;
	vars->h7 = 0x5be0cd19137e2179;
	vars->msg = prepare_sha512(inp, len);
	vars->new_len = find_len512(len);
	return (create_k512());
}
